  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="rounded-[2.2rem] border border-black/10 bg-white p-5 md:p-8">
        ...
        <p className="mt-3 text-sm font-bold leading-6 text-black/50">A Duolingo-like path, but built for diaspora language identity instead of streak anxiety.</p>
            ...
            return (
              <div key={family.id} className="grid grid-cols-[minmax(0,1fr)_76px] items-center gap-4 md:grid-cols-[minmax(0,1fr)_82px_minmax(0,1fr)]">
                <div className={index % 2 === 0 ? "text-right" : "md:col-start-3"}>
                  ...
                  <p className="text-xs font-bold text-black/45">{complete ? "Complete" : family.category}</p>
                </div>
                <div className={["row-start-1 mx-auto grid size-18 place-items-center rounded-full border-4 font-[var(--ethiopic)] text-3xl font-black md:col-start-2 md:size-20", complete ? "border-black bg-black text-white" : "border-[#58cc02] bg-white text-black"].join(" ")}>
      ...
      <aside className="grid content-start gap-4">
        ...
        <button onClick={onReset} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl border border-black/10 bg-white px-5 text-sm font-black text-black/65 transition hover:border-black hover:text-black">
          <RotateCcw size={17} />
