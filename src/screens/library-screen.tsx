...
export function LibraryScreen({ query, onQuery, onSpeak }: { query: string; onQuery: (query: string) => void; onSpeak: (text: string) => void }) {
  ...
  return (
    <div className="grid gap-5">
      <section className="rounded-[2rem] bg-black p-7 text-white md:p-10">
        ...
        <label className="mt-7 flex min-h-14 items-center gap-3 rounded-2xl bg-white px-4 text-black">
          <Search size={19} />
          <input
            value={query}
      ...
      <section className="grid gap-4 xl:grid-cols-[1fr_0.75fr]">
        <div className="grid gap-3">
          ...
          {filteredWords.map((word) => (
            <article key={word.id} className="rounded-[1.5rem] border border-black/10 bg-white p-5">
              <div className="flex items-start justify-between gap-4">
                ...
                <button onClick={() => onSpeak(word.amharic)} className="grid size-11 place-items-center rounded-2xl bg-black text-white">
                  <Volume2 size={18} />
                </button>
              </div>
        ...
        <aside className="grid content-start gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {filteredFamilies.map((family) => (
              <div key={family.id} className="rounded-[1.25rem] border border-black/10 bg-white p-4">
                <p className="font-[var(--ethiopic)] text-4xl font-black">{family.base}</p>
                <p className="mt-2 text-sm font-black">{family.name}</p>
                <p className="text-xs font-bold text-black/45">{family.orders.map((order) => order.fidel).join(" ")}</p>
              </div>
            ))}
          </div>
        </aside>
...
function Note({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-black/10 bg-[#fafafa] p-3">
      ...
      <p className="mt-2 text-sm font-bold leading-6 text-black/60">{value}</p>
    </div>
  );
}
