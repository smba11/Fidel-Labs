...
export function ConversationLessonScreen({
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        {conversations.map((lesson) => (
          <button
            ...
            className={[
              ...
              lesson.id === activeConversation.id ? "border-black bg-black text-white" : "border-black/10 bg-white hover:border-black",
            ].join(" ")}
          >
      ...
      <section className="rounded-[2.2rem] border border-black/10 bg-white p-5 md:p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-black/35">{activeConversation.level} conversation</p>
            <h2 className="mt-3 text-5xl font-black tracking-tight">{activeConversation.title}</h2>
            <p className="mt-3 text-sm font-bold text-black/50">{activeConversation.scenario}</p>
          </div>
          <button
            onClick={() => onComplete(activeConversation.id, activeConversation.xp)}
            className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-black px-5 text-sm font-black text-white"
          >
            <Play size={17} />
        ...
        <div className="mt-8 grid gap-4">
          {activeConversation.lines.map((line, index) => (
            <article key={`${line.amharic}-${index}`} className="rounded-[1.5rem] border border-black/10 bg-[#fafafa] p-5">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.2em] text-black/35">{line.speaker} · {line.tone}</p>
                  ...
                  <p className="mt-1 text-sm font-bold text-black/55">{line.english}</p>
                </div>
                <button onClick={() => onSpeak(line.amharic)} className="grid size-12 place-items-center rounded-2xl bg-black text-white">
                  <Volume2 size={19} />
                </button>
              </div>
              <p className="mt-4 rounded-2xl border border-black/10 bg-white p-3 text-sm font-bold leading-6 text-black/55">{line.note}</p>
            </article>
          ))}
        </div>
      </section>
